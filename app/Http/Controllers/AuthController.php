...
class AuthController extends Controller
{
    ...
    public function proses_login(Request $request){
        ...
        //validasi nya username dan password wajib di isi
        $request->validate([
            'username' => 'required',
            'password' => 'required',
        ]);
        ...
        if(Auth::attempt($credential)){
            //kalau berhasil simpan data user admin maka diarahkan ke halaman admin
            $user = Auth::user();
        ...
        //jika ga ada data user yang valid maka kembalikan lagi ke halaman login
            //pastikan kirim pesan error juga kalau gagal login
    ...
    public function proses_register(Request $request){
        ...
        //kalau gagall kembali ke halaman register dengan memunculkan pesan error
        if($validator->fails()){
            return redirect('/register')
            ->withErrors($validator)
            ->withInput();
        }
        //kalau berhasil isi level $ hash passwornya ya biar secure
        $request['level_id'] = '2';
    ...
    public function logout(Request $request){
        ...
        //jalan kan fungsi logout pada auth
        Auth::logout();
